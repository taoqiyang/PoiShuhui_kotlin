...
class AnotherAdapter : Adapter<ViewHolder>() {
  ...
  fun append(newItems: List<Any>){
    if(newItems.isEmpty()){
      return
    }
    if (types.contains(newItems.get(0).javaClass)) {
      items.addAll(newItems)
      notifyItemRangeInserted(items.size - newItems.size, newItems.size)
    } else {
      throw TypeNotBindException("can not find binder of this type : ${newItems.get(0).javaClass}")
    }
  }
